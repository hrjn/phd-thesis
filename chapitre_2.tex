	...
	\For{$k=1, \dots, K$}
		...
		\For{$l=0, \dots, k$}
			\For{$i=1, \dots, N_p$}
				...
				\EndFor
			\EndFor
		\State Mettre à jour la loi de proposition en calculant $\alpha^{k+1}$ et $\nu^{k+1}$ à partir de $\alpha^k$, $\nu^k$ , de la concaténation des poids recyclés normalisés $\tilde{w}_0, \dots, \tilde{w}_k$ \NdFS{et des particules correspondantes $\theta_0, \cdots, \theta_K$}
		\EndFor
