...
\newpage \NdFS{La loi $p(\VecThetaMaj|\VecObs)$ est dite \textit{jointe}, car elle regroupe tous les paramètres de la source autour d'une unique loi a posteriori. Par définition des probabilités conditionnelles, cette loi peut s'écrire de la façon suivante:}
...
{
	Etant donnés les paramètres $\VecMeanQ$ et $\MatSigma_q$ définis à l'équation \eqref{eq_AE_8} et la formulation du modèle de l'équation \eqref{eq_AE_3}, \NdFS{la vraisemblance marginale peut s'écrire sous la forme suivante:
...
\end{equation}
 ...
 On peut voir sur la figure \ref{fig_5_AE} que l'AMIS fournit une bonne estimation de la position de la source dans chacun des \textit{trials}.\\
  ...
  \NdFS{Afin de visualiser l'estimation du profil d'émission, on utilise également une approche basée sur le MMSE. Le vecteur $\widehat{\VecQSource}$ estimé est obtenu depuis l'équation \eqref{eq_estimateurs_mmse}, et la matrice de covariance estimée associée est donnée par:
